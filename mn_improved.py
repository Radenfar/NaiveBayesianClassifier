class ImprovedNaiveBayes:
    def __init__(self, validation_data_split: float, alpha: float, stop_word_proportion: float, auto_load_data: bool = True) -> None:
        if auto_load_data:
            self.training_data: MN_DataModel = MN_DataModel(
                data_path=os.path.join(os.path.join("data", "trg.csv")),
                do_shuffle=True
            )
            self.testing_data: MN_DataModel = MN_DataModel(
            ...
            self.training_data = MN_DataModel('')
            self.testing_data = MN_DataModel('')
            self.validation_data = None
            self.alpha = alpha
            self.vocab_size = 0
            self.classes = []
    ...
    def set_data(self, training_data: MN_DataModel = None, testing_data: MN_DataModel = None, validation_data: DataModel = None) -> None:
        if training_data:
